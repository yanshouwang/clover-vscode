...
import { flatMap, notUndefined } from "../util";
import { SimpleTimeBasedCache } from "../util/cache";
import { findProjectFolders, forceWindowsDriveLetterToUppercase, fsPath } from "../util/fs";
import { isKnownCloudEditor } from "./cloud_util";
...
export async function getAllProjectFolders(
    ...
    if (inProgressProjectFolderSearch === resultsPromise) { inProgressProjectFolderSearch = undefined; }
    return results;
}
...
export function showCode(editor: TextEditor, displayRange: Range, highlightRange: Range, selectionRange?: Range): void {
    if (selectionRange) { editor.selection = new Selection(selectionRange.start, selectionRange.end); }
...
class EnvUtils {
    ...
    public async exposeUrl(urlString: string, logger: Logger = nullLogger): Promise<string> {
        ...
        if (isWebSocket) { fakeScheme = isSecure ? "https" : "http"; }
