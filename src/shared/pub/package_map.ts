...
import { findFileInAncestor, uriToFilePath } from "../util";
import { normalizeSlashes } from "../util/fs";
...
export abstract class PackageMap {
    ...
    public static loadForProject(logger: Logger, projectFolder: string): PackageMap {
        ...
        for (const p of paths) {
            ...
            if (fs.existsSync(fullP)) { return this.load(logger, fullP); }
        }
        return new MissingPackageMap();
    }

    public static load(logger: Logger, file: string | undefined): PackageMap {
        if (!file) { return new MissingPackageMap(); }
        try {
            if (path.basename(file).toLowerCase() === ".packages") { return new DotPackagesPackageMap(file); }
            else { return new PackageConfigJsonPackageMap(logger, file); }
        } catch (e) {
            logger.error(e);
            return new MissingPackageMap();
        }
    ...
    public resolvePackageUri(uri: string): string | undefined {
        ...
        let name: string = uri;
        if (name.startsWith("package:")) { name = name.substring(8); }
        ...
        if (location) { return path.join(location, rest); }
    ...
    constructor(file?: string) {
        ...
        for (let line of lines) {
            ...
            if (index !== -1) {
                ...
                if (rest.startsWith("file:")) { this.map[name] = uriToFilePath(rest); }
                else { this.map[name] = path.join(this.localPackageRoot, rest); }
            }
        }
    }
