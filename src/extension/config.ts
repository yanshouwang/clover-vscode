...
import { NullAsUndefined, nullToUndefined } from "../shared/util";
import { createFolderForFile, resolvePaths } from "./util";
import { setupToolEnv } from "./util/processes";
...
class Config {
    ...
    get flutterHotReloadOnSave(): "never" | "manual" | "manualIfDirty" | "all" | "allIfDirty" {
        ...
        if (value === true) { return "manual"; }
        else if (value === false) { return "never"; }
        else if (value === "always") { return "all"; }
        else { return value; }
    }
    ...
    get previewSdkDaps(): boolean { return this.getConfig<boolean>("previewSdkDaps", false); }
