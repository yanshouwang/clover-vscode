...
import { fsPath, getRandomInt, hasPubspec, isFlutterProjectFolder, isWithinPath, mkDirRecursive } from "../shared/util/fs";
import { isDartWorkspaceFolder } from "../shared/vscode/util";
...
export function resolvePaths<T extends string | undefined>(p: T): string | (undefined extends T ? undefined : never) {
	...
	if (!path.isAbsolute(p) && workspace.workspaceFolders && workspace.workspaceFolders.length) { return path.join(fsPath(workspace.workspaceFolders[0].uri), p); }
	return p;
}
...
export function homeRelativePath(p?: string) {
	...
	if (isWithinPath(p, homedir)) { return path.join("~", path.relative(homedir, p)); }
	return p;
}
...
export function createFolderForFile(file?: string): string | undefined {
	try {
		...
		if (!fs.existsSync(folder)) { mkDirRecursive(folder); }
...
export function isAnalyzable(file: { uri: Uri, isUntitled?: boolean, languageId?: string }): boolean {
	if (file.isUntitled || !fsPath(file.uri) || file.uri.scheme !== "file") { return false; }
...
export function hasTestNameFilter(...argss: Array<string[] | undefined>) {
	for (const args of argss) {
		if (args && (args.includes("--name") || args.includes("--pname"))) { return true; }
	}
	return false;
}
...
export function isValidEntryFile(file: string | undefined) {
	if (!file || !isDartFile(file)) { return false; }

	// When in a no-folder workspace, all Dart files are considered runnable.
	if (!workspace.workspaceFolders?.length) { return true; }
...
export function openLogContents(logType = `txt`, logContents: string, tempPath?: string) {
	if (!tempPath) { tempPath = path.join(os.tmpdir(), `log-${getRandomInt(0x1000, 0x10000).toString(16)}.${logType}`); }
...
export function getExcludedFolders(f: WorkspaceFolder | undefined): string[] {
	...
	if (!excludedForWorkspace || !Array.isArray(excludedForWorkspace)) { return []; }

	const workspacePath = fsPath(f.uri);
	return excludedForWorkspace.map((folder) => {
		// Handle both relative and absolute paths.
		if (!path.isAbsolute(folder)) { folder = path.join(workspacePath, folder); }
		return folder;
	});
}